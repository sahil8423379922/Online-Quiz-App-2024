...
class Plan extends StatefulWidget {
  final User user;
  const Plan({super.key, required this.user});
...
class _PlanState extends State<Plan> {
  ...
  Future<void> send_subscription_info(mode) async {
    if (mode == 'paid') {
      await firestore
          .collection('subscription')
          .doc(widget.user.email)
          .set({'status': 'paid', 'date': DateTime.now()});
    } else {
      await firestore
          .collection('subscription')
          .doc(widget.user.email)
          .set({'status': 'free', 'date': DateTime.now()});
    }
  }
}
